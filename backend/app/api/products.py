...
from app.scrapers.collector import collect_fast
...
def _translate_query(q: str) -> str:
    try:
        ...
        return detected if detected and detected.lower() != q.lower() else q
    except Exception:
        return q
...
@limiter.limit("120/minute")   # autocomplete-safe; collect=true separately guarded below
async def search_products(
    ...
    if collect and len(q.strip()) >= _MIN_COLLECT_LEN:
        ...
        try:
            ...
            task = run_collection_slow.delay(q)
            job_id = task.id
            collecting = True
        except Exception:
            # Celery 미연결 환경 (테스트 등) — 폴링 없이 fast 결과만 반환
            pass
...
async def get_product_events(
    ...
    events_out = [
        SaleEventOut(
            ...
            scraped_name=getattr(e, "scraped_name", None),
            is_bundle=bool(getattr(e, "is_bundle", False)),
        )
        for e, p in rows
    ]
