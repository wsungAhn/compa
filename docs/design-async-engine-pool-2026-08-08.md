...
**정정된 최악 계산** (r3에서 한 번 더 틀렸고 r4에서 재정정 — 아래 주석 참조):

| 상태 | worker | API | 합계 |
|---|---:|---:|---:|
| 정상 운영 | 2 × 30 = **60** | 요청당 1 (`get_db`) + `collect_fast` 0 → 소수 | **~65 < 97 ✅** |
| Redis 불통 **지속** | 0 (태스크 미수신) | 서로 다른 쿼리 N건 × 30 | 30N |
| Redis 불통 **전환 순간** | 최대 60 (이미 받아 실행 중인 태스크) | 30N | **60 + 30N** |

> r3에서 나는 "디스패치 실패와 worker 부하는 양립 불가"라고 썼다. **틀렸다**(r4 지적).
> Redis가 끊기는 순간 worker가 **이미 수신해 실행 중인** 태스크는 계속 돌고, 그 사이
> API는 폴백으로 넘어간다. 이 전환 구간에서 두 부하는 겹친다. **N ≥ 2면 120 > 97.**

**결론 — 상한을 보장하지 못한다는 사실을 그대로 기록한다.** 정상 운영(~65)은 안전하나,
Redis 장애 전환 구간에서 서로 다른 검색어 2건 이상이 겹치면 `too many connections`가
날 수 있다. 이를 "안전하다"고 쓰지 않는다.

**그럼에도 전역 NullPool을 유지하는 이유**:
1. 이 경합은 **Redis 장애라는 이미 비정상인 구간**에서만 열리고, 그 구간엔 수집
   파이프라인이 어차히 멈춰 있다. PG가 커넥션을 거부해도 새로 깨지는 불변식이 없다
2. 대안(엔진 분리)은 **무증상 재발 모드**를 갖는다(§5 대안표) — 상시 정상 운영에서
   조용히 P0가 돌아오는 쪽이, 장애 구간에서 시끄럽게 거부되는 쪽보다 나쁘다
3. 트래픽이 사실상 없는 현 단계에서 N ≥ 2는 관측된 적이 없다 — 관측되면 트리거다
...
4. **worker/beat 재시작** (사용자 승인됨, 2026-08-08 — **단 §7.0 게이트 통과 후**) —
   ...
   **API는 이번 세션에서 재시작하지 않는다** (사용자 승인 범위가 worker/beat뿐).
   ...
   재부팅·수동 kickstart 무엇이든. 그것은 막지 않으며 막을 수단도 두지 않는다(§5.1
   결론대로 정상 운영 ~65 < 97).

   **다만 그 결과 이번 관찰 창은 API 쪽 영향(+5.5ms, 커넥션)을 검증하지 못한다**
   (감사 r2·r4 지적). API가 다음에 재시작될 때 §7.1 샘플링을 한 번 더 돌려
   피크를 확인할 것 — 핸드오프에 후속으로 남긴다.
5. 재시작 후 **최소 20분 관찰**하고 `worker.err.log`의 태스크별 성공/실패를 §1 표와
   ...
   **"태스크 성공"만 세면 안 된다 (감사 r4)**: 코드 곳곳이 `except Exception:
   continue/pass`로 DB 오류를 삼킨다 — `_collect_all`(`collect.py:34`),
   `_collect_in_background`(`products.py:76`), `_collect_platform`. 즉 **내부 전량이
   실패해도 태스크는 성공으로 집계된다.** 판정에 아래를 함께 본다:

   | 지표 | 확인 방법 |
   |---|---|
   | 태스크 성공/실패 | `worker.err.log` 재집계 (오프셋 이후) |
   | **경고 로그** | 같은 창의 `logger.warning` 건수 — 삼켜진 실패의 유일한 흔적 |
   | **반환 count** | 태스크 반환값이 0이 아닌지 (0이면 "성공했지만 아무것도 못 했다") |
   | **행 delta** | `sale_events`·`social_posts` 행 수를 창 전후로 비교 |
...
샘플링 창은 §7-5의 관찰 창(**최소 1시간**)과 같아야 한다(감사 r4) — 가장 넓은 팬아웃이
도는 시점과 어긋나면 피크를 놓친다.

```bash
# 60분간 5초 간격으로 compa 커넥션 수를 샘플링 → 최댓값 기록
