...
오케스트레이터(sonnet)는 계획·검토·판단을 담당하고, 실제 코드 작성은 반드시 전문 서브에이전트(haiku)에 위임한다.

| 작업 | 담당 |
|------|------|
| 요구사항 분석, 설계, 아키텍처 결정 | 오케스트레이터 (sonnet) |
| 코드 리뷰, 버그 원인 분석 | 오케스트레이터 (sonnet) |
| 스크래퍼 코드 작성 / 테스트 작성 | `scraper` 에이전트 (haiku) |
| AI 파이프라인 코드 작성 | `ai-pipeline` 에이전트 (haiku) |
| React 컴포넌트 / 프론트엔드 코드 작성 | `frontend` 에이전트 (haiku) |
| 반복적인 보일러플레이트, 모델/스키마 파일 | 해당 도메인 에이전트 (haiku) |

> 코드를 직접 작성하기 전에 항상 멈추고, 해당 도메인의 서브에이전트로 위임할 것.
> 단, 5줄 이하의 간단한 수정, 설정 파일 편집, 버그 핫픽스는 직접 처리 가능.
...
- [x] M4: 중국·소셜 + 수익화 (Tmall·샤오홍슈, 소셜 수집·추출 파이프라인, 제휴링크+프리미엄 게이트+광고 슬롯 — 광고 네트워크/PG 연동은 배포·심사 후 별도)
